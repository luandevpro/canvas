...
class Handler implements HandlerOptions {
   ...
   * @param {string} [option={ name: 'New Image', format: 'png', quality: 1 }]
   */
  public saveCanvasImage = (option?: DataURLOption) => {
    const defaultOption: DataURLOption = {
      name: "New Image",
      format: "png",
      quality: 1,
    };
  ...
  public renderPngImage = (option?: DataURLOption) => {
    const defaultOption: DataURLOption = {
      name: "New Image",
      format: "png",
      quality: 1,
      ...
      width: this.workarea.getScaledWidth(),
      height: this.workarea.getScaledHeight(),
    ...
    const dataUrl = this.canvas.toDataURL(option);
    this.zoomHandler.zoomWithRatio(zoomRatio);
    this.canvas.renderAll();
    if (dataUrl) {
      const anchorEl = document.createElement("a");
      anchorEl.href = dataUrl;
